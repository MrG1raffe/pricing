...
class ForwardContract:
    ...
    def __init__(
        self,
        name: str,
        observation_date: Union[str, np.datetime64] = None,
        F0: float_ = None,
    ):
        """
        ...
        """
        if len(name) not in (4, 5, 6) and not name.startswith("D"):
        ...
        if not name[:3] in MONTH_DICT.keys():
            ...
            elif name.lower().startswith("gcal"):
                year = "20" + name[4:6]
                self.delivery_start_date = pd.Timestamp(np.datetime64(year + '-10-01'))
                self.delivery_end_date = pd.Timestamp(self.delivery_start_date) + pd.DateOffset(years=1)
                self.forward_type = "gcal"
